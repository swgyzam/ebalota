...
// Get scope information from session (set in admin_verify_token.php)
 $scope_category = $_SESSION['scope_category'] ?? '';
 $assigned_scope = $_SESSION['assigned_scope'] ?? '';
 $assigned_scope_1 = $_SESSION['assigned_scope_1'] ?? '';
 $scope_details = $_SESSION['scope_details'] ?? [];
 $admin_status = $_SESSION['admin_status'] ?? 'inactive';

// Validate this is a College Admin (Academic-Student or Academic-Faculty)
if (!in_array($scope_category, ['Academic-Student', 'Academic-Faculty'])) {
...
if (!empty($allowed_colleges) && !in_array('ALL', $allowed_colleges) && !in_array(strtoupper($assigned_scope), $allowed_colleges)) {
    header('Location: admin_analytics_college.php');
    exit();
}

// Get allowed courses/departments from admin's scope
 $allowed_courses = [];
 $allowed_departments = [];

if ($scope_category === 'Academic-Student') {
    // For Academic-Student, get allowed courses from scope_details
    $allowed_courses = $scope_details['courses'] ?? [];
    // If "All" courses are selected, get all courses for this college
    if (isset($scope_details['courses_display']) && $scope_details['courses_display'] === 'All') {
        require_once 'admin_functions.php';
        $all_courses = getCoursesByCollege($assigned_scope);
        $allowed_courses = array_keys($all_courses);
    }
} elseif ($scope_category === 'Academic-Faculty') {
    // For Academic-Faculty, get allowed departments from scope_details
    $allowed_departments = $scope_details['departments'] ?? [];
    // If "All" departments are selected, get all departments for this college
    if (isset($scope_details['departments_display']) && $scope_details['departments_display'] === 'All') {
        require_once 'admin_functions.php';
        $academic_departments = getAcademicDepartments();
        $college_departments = $academic_departments[$assigned_scope] ?? [];
        $allowed_departments = array_keys($college_departments);
    }
}
...
// UPDATED COURSE MAPPINGS - Match election creation format exactly
 $course_map = [
    ...
    // ... (rest of course mappings remain the same)
];
...
// Department abbreviation mapping
 $department_abbr_map = [
    ...
    // ... (rest of department mappings remain the same)
];
...
// Define college departments structure
 $collegeDepartments = [
    "CAFENR" => [
        ...
        // ... (rest of departments remain the same)
    ],
    // ... (rest of colleges remain the same)
];
...
// Define college courses structure
 $collegeCourses = [
    "CEIT" => [
        ...
        // ... (rest of courses remain the same)
    ],
    // ... (rest of colleges remain the same)
];
...
// ===== GET ELIGIBLE VOTERS COUNT =====
 ...
 $params = [strtoupper($assigned_scope)];

// Get allowed filters from election
 $election_allowed_courses = array_filter(array_map('strtoupper', array_map('trim', explode(',', $election['allowed_courses'] ?? ''))));

// Apply course filter if specified in election
if (!empty($election_allowed_courses) && !in_array('ALL', $election_allowed_courses)) {
    ...
    $course_list = [];
    foreach ($election_allowed_courses as $course) {
...
// Apply admin's scope restrictions
if ($scope_category === 'Academic-Student' && !empty($allowed_courses)) {
    // Filter by allowed courses
    $allowed_course_names = [];
    foreach ($allowed_courses as $course_code) {
        // Convert course code to full name for query
        foreach ($course_map as $full_name => $code) {
            if ($code === $course_code) {
                $allowed_course_names[] = strtolower($full_name);
                break;
            }
        }
    }
    
    if (!empty($allowed_course_names)) {
        $placeholders = implode(',', array_fill(0, count($allowed_course_names), '?'));
        $conditions[] = "LOWER(course) IN ($placeholders)";
        $params = array_merge($params, $allowed_course_names);
    }
} elseif ($scope_category === 'Academic-Faculty' && !empty($allowed_departments)) {
    // For faculty, we need to filter by department
    $allowed_department_names = [];
    foreach ($allowed_departments as $dept_code) {
        // Convert department code to full name for query
        foreach ($department_abbr_map as $full_name => $code) {
            if ($code === $dept_code) {
                $allowed_department_names[] = $full_name;
                break;
            }
        }
    }
    
    if (!empty($allowed_department_names)) {
        $placeholders = implode(',', array_fill(0, count($allowed_department_names), '?'));
        $conditions[] = "department1 IN ($placeholders)";
        $params = array_merge($params, $allowed_department_names);
    }
}
...
";
 ...
 $params = [$electionId, strtoupper($assigned_scope)];
...
if ($scope_category === 'Academic-Student' && !empty($allowed_courses)) {
    ...
    if (!empty($allowed_department_names)) {
        $placeholders = implode(',', array_fill(0, count($allowed_department_names), '?'));
        $sql .= " AND u.department1 IN ($placeholders)";
        $params = array_merge($params, $allowed_department_names);
    }
}
...
// Get list of departments and courses for this college
 $departmentsList = isset($collegeDepartments[$assigned_scope]) ? $collegeDepartments[$assigned_scope] : [];
 $coursesList = isset($collegeCourses[$assigned_scope]) ? $collegeCourses[$assigned_scope] : [];

// Filter departments and courses based on admin's scope
if ($scope_category === 'Academic-Faculty' && !empty($allowed_departments)) {
    // Filter departments based on admin's allowed departments
    $filteredDepartments = [];
    foreach ($departmentsList as $department) {
        foreach ($department_abbr_map as $full_name => $code) {
            if ($full_name === $department && in_array($code, $allowed_departments)) {
...
if ($scope_category === 'Academic-Student' && !empty($allowed_courses)) {
    // Filter courses based on admin's allowed courses
    $filteredCourses = [];
    foreach ($coursesList as $course) {
        foreach ($course_map as $full_name => $code) {
            if ($full_name === $course && in_array($code, $allowed_courses)) {
...
function formatScopeForDisplay($category, $scope, $scope1, $details) {
    switch ($category) {
        case 'Academic-Student':
            $college = $scope;
            $courses = $scope1;
            if (empty($courses) || $courses === 'All') {
                return "$college - All Courses";
            }
            return "$college - Courses: $courses";
            
        case 'Academic-Faculty':
            $college = $scope;
            $departments = $scope1;
            if (empty($departments) || $departments === 'All') {
                return "$college - All Departments";
            }
            return "$college - Departments: $departments";
 ...
 $pageTitle = htmlspecialchars($assigned_scope) . ' Election Analytics - ' . htmlspecialchars($scopeDisplay);
    ...
    <div class="max-w-7xl mx-auto">
              ...
              <div class="flex items-center mb-3">
                ...
                <div>
                  ...
                  <h1 class="text-3xl font-bold text-white leading-tight">
                    <?= htmlspecialchars($assigned_scope) ?> Election Analytics
                  </h1>
      ...
      <div class="bg-white rounded-xl shadow-md overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
          ...
          <p class="text-sm text-gray-500 mt-1"><?= htmlspecialchars($scopeDisplay) ?> (by department and course)</p>
        </div>
