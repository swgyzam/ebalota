<?php
// super_admin_change_password_modal.php
...
<div id="superAdminChangePasswordModal"
  ...
  <div class="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col relative">
      ...
      <!-- Close button -->
      <button type="button"
              onclick="closeSuperAdminChangePasswordModal()"
      ...
      <div class="pt-7 pb-4 px-8 border-b border-gray-100">
        <div class="text-center">
          ...
          <p class="text-gray-600 mt-1 text-xs sm:text-sm">
            Update your password. You’ll need this next time you log in.
          </p>

          <!-- ERROR -->
          <div id="superAdminPasswordErrorTop"
               class="hidden mt-3 bg-red-100 border border-red-500 text-red-800 text-xs sm:text-sm px-4 py-2 rounded-md shadow-sm">
              <span id="superAdminPasswordErrorTopText"></span>
          </div>

          <!-- SUCCESS -->
          <div id="superAdminPasswordSuccess"
          ...
          <!-- Locked hint + cooldown -->
          <p id="superAdminLockedHint"
      ...
      <form id="superAdminChangePasswordForm"
          ...
          <div>
              ...
              <div class="relative">
                  ...
                  <button type="button"
                          ...
                          data-toggle-password="superAdminCurrentPassword">
          ...
          <div>
              ...
              <div class="relative">
                  ...
                  <button type="button"
                          class="absolute inset-y-0 right-3 flex items-center text-gray-400 hover:text-gray-600"
                          data-toggle-password="superAdminNewPassword">
                      <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <div class="mt-2">
                  <div class="flex items-center text-[11px] text-gray-500 mb-1">
                      <span id="superAdminPasswordStrength" class="font-medium">Password strength:</span>
                      <div class="ml-2 flex-1 bg-gray-200 rounded-full h-2">
                          <div id="superAdminStrengthBar"
                               class="h-2 rounded-full password-strength-bar"
                               style="width: 0%"></div>
                      </div>
                  </div>
                  <ul class="text-[11px] text-gray-500 space-y-1">
                      <li id="superAdminLengthCheck" class="flex items-center">
                      ...
                      <li id="superAdminUppercaseCheck" class="flex items-center">
                          <svg class="h-3.5 w-3.5 mr-1 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M5 13l4 4L19 7" />
                          </svg>
                      ...
                      <li id="superAdminNumberCheck" class="flex items-center">
                          <svg class="h-3.5 w-3.5 mr-1 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M5 13l4 4L19 7" />
                          </svg>
                      ...
                      <li id="superAdminSpecialCheck" class="flex items-center">
                          <svg class="h-3.5 w-3.5 mr-1 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M5 13l4 4L19 7" />
                          </svg>
          ...
          <div>
              ...
              <div class="relative">
                  ...
                  <button type="button"
                          class="absolute inset-y-0 right-3 flex items-center text-gray-400 hover:text-gray-600"
                          data-toggle-password="superAdminConfirmPassword">
                      <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <div id="superAdminMatchError" class="mt-1 text-[11px] text-red-500 hidden">
      ...
      <div class="px-8 pb-5 pt-1 border-t border-gray-100">
          <div class="flex justify-center">
              <button type="submit"
                      form="superAdminChangePasswordForm"
                      id="superAdminChangePasswordBtn"
                  ...
                  <span id="superAdminChangePasswordBtnText">Update Password</span>
                  <svg id="superAdminChangePasswordLoader"
...
<script>
function openSuperAdminChangePasswordModal() {
  const modal = document.getElementById('superAdminChangePasswordModal');
  if (!modal) return;
  modal.classList.remove('hidden');
  modal.classList.add('flex');
  document.body.style.overflow = 'hidden';

  if (typeof restoreSuperAdminPwdCooldownIfNeeded === 'function') {
    restoreSuperAdminPwdCooldownIfNeeded();
  }
}

function closeSuperAdminChangePasswordModal() {
  const modal = document.getElementById('superAdminChangePasswordModal');
  if (!modal) return;
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const form         = document.getElementById('superAdminChangePasswordForm');
  if (!form) return;

  const currentInput = document.getElementById('superAdminCurrentPassword');
  const newInput     = document.getElementById('superAdminNewPassword');
  const confirmInput = document.getElementById('superAdminConfirmPassword');

  const strengthBar  = document.getElementById('superAdminStrengthBar');
  const strengthText = document.getElementById('superAdminPasswordStrength');
  const lengthCheck  = document.getElementById('superAdminLengthCheck');
  const upperCheck   = document.getElementById('superAdminUppercaseCheck');
  const numberCheck  = document.getElementById('superAdminNumberCheck');
  const specialCheck = document.getElementById('superAdminSpecialCheck');

  const matchError   = document.getElementById('superAdminMatchError');
  const okBox        = document.getElementById('superAdminPasswordSuccess');
  const lockedHint   = document.getElementById('superAdminLockedHint');
  const cooldownText = document.getElementById('superAdminCooldownText');

  const topErrBox    = document.getElementById('superAdminPasswordErrorTop');
  const topErrText   = document.getElementById('superAdminPasswordErrorTopText');

  const btn          = document.getElementById('superAdminChangePasswordBtn');
  const btnText      = document.getElementById('superAdminChangePasswordBtnText');
  const loader       = document.getElementById('superAdminChangePasswordLoader');

  let cooldownInterval = null;

  const COOLDOWN_KEY = 'superAdminPwdCooldownUntil';
  ...
  function startCooldown(untilTimestamp) {
    ...
    localStorage.setItem(COOLDOWN_KEY, String(untilTimestamp));
  ...
  window.restoreSuperAdminPwdCooldownIfNeeded = function restoreSuperAdminPwdCooldownIfNeeded() {
  ...
  window.restoreSuperAdminPwdCooldownIfNeeded();
