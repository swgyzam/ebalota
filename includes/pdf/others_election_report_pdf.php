<?php
/**
 * others_election_report_pdf.php
 *
 * OTHERS (custom group) election report generator.
 * For elections with election_scope_type = 'Others'
 * and voters under SCOPE_OTHERS.
 */

if (!function_exists('generateOthersElectionReportPDF')) {
    function generateOthersElectionReportPDF(PDO $pdo, int $electionId, int $adminUserId, int $scopeId): void
    {
        /* ======================================================
           1. LOAD DEPENDENCIES
           ====================================================== */
        ...
        // Guard: must be an Others election under this scope seat
        if (($election['election_scope_type'] ?? '') !== 'Others') {
            die('This election is not an Others election.');
        }

        if ((int)($election['owner_scope_id'] ?? 0) !== $scopeId) {
            die('You do not have permission to generate this Others report.');
        }
        ...
        // Look up this specific Others seat to get scope details
        $mySeat    = null;
        $otherSeats = getScopeSeats($pdo, SCOPE_OTHERS);
        foreach ($otherSeats as $seat) {
        ...
        if (!$mySeat) {
            die('Others scope seat not found.');
        }

        $scopeDetails = [];
        if (!empty($mySeat['scope_details'])) {
            if (is_array($mySeat['scope_details'])) {
                // galing sa getScopeSeats → array na
                $scopeDetails = $mySeat['scope_details'];
            } elseif (is_string($mySeat['scope_details'])) {
                // fallback kung sakaling may old code na string pa
                $decoded = json_decode($mySeat['scope_details'], true);
                if (is_array($decoded)) {
        ...
        // Try to get a nice group name from scope details, with fallbacks
        $groupName = $scopeDetails['group_name']
            ?? $scopeDetails['display_name']
            ?? $scopeDetails['label']
            ?? ($election['title'] ?? 'Others Group');

        /* ======================================================
           4. ELIGIBLE VOTERS (OTHERS under this seat) + TURNOUT
           ====================================================== */

        $yearEnd = $election['end_datetime'] ?? null;

        // All scoped Others voters as of this election's end
        ...
        // Eligible set: all scoped Others under this seat
        $eligibleVoters = [];
        $hasDepartment  = false;
        $hasPosition    = false;
        $hasStatus      = false;
        ...
        $totalEligibleVoters = count($eligibleVoters);
        ...
        $totalVotesCast = 0;
        foreach ($eligibleVoters as $uid => $_row) {
        ...
        // - facultyCollegeBuckets: college (department code) for Faculty
        // - nonAcadDeptBuckets   : non-academic department (code) for Non-Academic
        $positionBuckets       = [];
        $statusBuckets         = [];
        $facultyCollegeBuckets = [];
        $nonAcadDeptBuckets    = [];

        foreach ($eligibleVoters as $uid => $v) {
            $pos      = $v['position']    ?: '';   // 'academic', 'non-academic', or ''
            $stat     = $v['status']      ?: '';   // Regular / Part-time / etc.
            $college  = $v['department']  ?: '';   // for faculty: college code (CEIT, CAS, ...)
            $deptCode = $v['department']  ?: '';   // for non-ac: ADMIN, LIBRARY, FINANCE, ...
            $deptFull = $v['department1'] ?: '';   // full dept name (mainly for academic), optional
            ...
            // -----------------------------
            if ($hasStatus && $stat !== '') {
                if (!isset($statusBuckets[$stat])) {
                    $statusBuckets[$stat] = ['eligible' => 0, 'voted' => 0];
                }
                $statusBuckets[$stat]['eligible']++;
                if (isset($votedSet[$uid])) {
                    $statusBuckets[$stat]['voted']++;
                }
            }
            ...
            // -----------------------------
            if ($hasDepartment && $pos === 'academic') {
                $collegeKey = $college !== '' ? $college : 'UNSPECIFIED';

                if (!isset($facultyCollegeBuckets[$collegeKey])) {
                    $facultyCollegeBuckets[$collegeKey] = ['eligible' => 0, 'voted' => 0];
                }
                $facultyCollegeBuckets[$collegeKey]['eligible']++;
                if (isset($votedSet[$uid])) {
                    $facultyCollegeBuckets[$collegeKey]['voted']++;
                }
            }

            // -----------------------------
            // 4) Non-Academic – by department (code)
            // -----------------------------
            if ($hasDepartment && $pos === 'non-academic') {
                $deptKey = $deptCode !== '' ? $deptCode : 'Unspecified';

                if (!isset($nonAcadDeptBuckets[$deptKey])) {
                    $nonAcadDeptBuckets[$deptKey] = ['eligible' => 0, 'voted' => 0];
                }
                $nonAcadDeptBuckets[$deptKey]['eligible']++;
                if (isset($votedSet[$uid])) {
                    $nonAcadDeptBuckets[$deptKey]['voted']++;
                }
            }
        }

        // ============================
        // Build summaries with % values
        // ============================

        // 5.1 Position summary
        $positionSummary = [];
        if ($hasPosition && !empty($positionBuckets)) {
        ...
        // 5.2 Status summary
        $statusSummary = [];
        if ($hasStatus && !empty($statusBuckets)) {
        ...
        // 5.3 Faculty college summary
        $facultyCollegeSummary = [];
        if (!empty($facultyCollegeBuckets)) {
        ...
        // 5.4 Non-academic department summary
        $nonAcadDeptSummary = [];
        if (!empty($nonAcadDeptBuckets)) {
        ...
        /* ======================================================
           8. INIT PDF
           ====================================================== */
        $pdf = new OthersPDF('P', 'mm', 'A4', true, 'UTF-8', false);
        $pdf->SetCreator('eBalota');
        $pdf->SetAuthor($adminName);
        $pdf->SetTitle('Others Election Report - ' . ($election['title'] ?? ''));
        $pdf->SetSubject('Others Election Report');
        ...
        /* ======================================================
           11. ELECTION SUMMARY (OVERALL TURNOUT)
           ====================================================== */
        $summaryHtml = '
        ...
        /* ======================================================
           13. TURNOUT BY STATUS (IF STATUS DATA EXISTS)
           ====================================================== */
        ...
        /* ======================================================
           14. TURNOUT BY COLLEGE – FACULTY (IF ANY)
           ====================================================== */
        ...
        /* ======================================================
           15. TURNOUT BY DEPARTMENT – NON-ACADEMIC EMPLOYEES (IF ANY)
           ====================================================== */
            ...
            foreach ($positions as $posName => $cands) {
                ...
                <table cellpadding="3" cellspacing="0" border="1" width="100%" style="font-size:9pt;border-color:#d1d5db;">
                  <tr style="background-color:#154734;color:#ffffff;">
                    ...
                    <th width="52%"><b>Candidate Name</b></th>
                    <th width="20%"><b>Total Votes</b></th>
                    <th width="20%"><b>% of Position Votes</b></th>
                  </tr>
                ';
        ...
        /* ======================================================
           18. OUTPUT PDF
           ====================================================== */
        $safeTitle = preg_replace('/[^A-Za-z0-9_\-]+/', '_', $election['title'] ?? 'others_election');
        $pdf->Output($safeTitle . '_others_report.pdf', 'I');
        exit;
    }
}
