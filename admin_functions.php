...
function getDepartmentCodeFromName($full_name, $college) {
    ...
    if (strpos($full_name, 'Department of') === 0) {
        ...
        foreach ($words as $word) {
            if (!in_array(strtolower($word), ['department','of','and'], true)) {
    ...
    switch ($scope_type) {
        case 'Academic-Student': {
            ...
            if (isset($details['courses_display']) && $details['courses_display'] === 'All') {
                return $college_code ? "$college_code - All courses" : 'All courses';
            }

            $courses = $details['courses'] ?? [];
            if (empty($courses)) {
                return $college_code ?: 'No specific courses assigned';
            }

            $course_codes = array_map(function($course) { return $course; }, $courses);
            return ($college_code ? "$college_code - " : '') . implode(', ', $course_codes);
        }
        ...
        case 'Academic-Faculty': {
            ...
            if (isset($details['departments_display']) && $details['departments_display'] === 'All') {
                return $college_code ? "$college_code - All departments" : 'All departments';
            }

            $departments = $details['departments'] ?? [];
            if (empty($departments)) {
                return $college_code ?: 'No specific departments assigned';
            }
            ...
            return ($college_code ? "$college_code - " : '') . implode(', ', $dept_codes);
        }
        ...
        case 'Non-Academic-Employee': {
            ...
            $dept_codes = array_map(function($dept) { return $dept; }, $departments);
...
// Function to get scope details for a specific category (for JS bootstrapping)
