...
// --- Force password change flag for this admin ---
...
// --- Patch 5: Super admin impersonation via ?scope_id= ---
$impersonatedScopeId = getImpersonatedScopeId();

if ($impersonatedScopeId !== null) {
    $seat = fetchScopeSeatById($pdo, $impersonatedScopeId);

    if (!$seat || $seat['scope_type'] !== 'Special-Scope') {
      die('Invalid scope for CSG dashboard.');
    }

    $scopeCategory = $seat['scope_type'];         // "Special-Scope"
    $adminStatus   = 'active';
    // optional: could read $seat['assigned_scope'] if needed, but CSG is global
}
...
// --- Fetch dashboard stats (NEW: use analytics_scopes) ---

// 1) All CSG-eligible voters via analytics_scopes (global students)
$scopedCSG = getScopedVoters(
    $pdo,
    SCOPE_SPECIAL_CSG,   // "Special-Scope" (CSG)
    $scopeId,            // CSG scope seat; getVotersSpecialCSGGlobal ignores scopeId but keeps API consistent
    [
        'year_end'      => null,
        'include_flags' => true,
    ]
...
// Total students
$total_voters = count($scopedCSG);

// 2) Scoped elections for Special-Scope (CSG)
...
// Use this for any election list in the UI
...
$turnoutDataByYear = computeTurnoutByYear(
    $pdo,
    SCOPE_SPECIAL_CSG,   // "Special-Scope" for CSG
    $scopeId,
    $scopedCSG,          // galing sa getScopedVoters step natin
    [
        'year_from' => null,
        'year_to'   => null,
    ]
...
// If walang data (no elections yet), gumawa ng stub para sa current year
if (empty($turnoutDataByYear)) {
    $cy = (int)date('Y');
    $turnoutDataByYear[$cy] = [
        'year'           => $cy,
        'total_voted'    => 0,
        'total_eligible' => 0,
        'turnout_rate'   => 0.0,
...
// Siguraduhin na may entry ang selectedYear at previousYear
...
// Ayusin ang order ng years (ascending)
...
// Basahin ?from_year= at ?to_year= (kung wala, gamitin buong range)
...
// Build turnoutRangeData para sa [fromYear..toYear]
$turnoutRangeData = [];
...
// Recompute growth_rate pero sa selected range lang
$prevY = null;
...
/* ==========================================================
PER-ELECTION STATS + ABSTAIN (analytics_scopes)
========================================================== */

// Per-election stats for current selected year (turnout + abstain per election)
$electionTurnoutStats = computePerElectionStatsWithAbstain(
    $pdo,
    SCOPE_SPECIAL_CSG,   // "Special-Scope" for CSG
    $scopeId,
    $scopedCSG,          // galing sa getScopedVoters(...)
    $selectedYear
);

// Abstain by year for full CSG scope (all years)
...
// Build abstainByYear EXACTLY over [fromYear..toYear] (same range as turnoutRangeData)
$abstainByYear = [];
for ($y = $fromYear; $y <= $toYear; $y++) {
    if (isset($abstainAllYears[$y])) {
        $abstainByYear[$y] = $abstainAllYears[$y];
    } else {
        // Stub year with zero values so charts/tables can still show it
        $abstainByYear[$y] = [
            'year'           => (int)$y,
            'abstain_count'  => 0,
            'total_eligible' => 0,
...
// Header scope display + impersonation flag (for consistent UI like other dashboards)
...
<header class="w-full fixed top-0 left-64 h-16 shadow z-10 flex items-center justify-between px-6" style="background-color:var(--cvsu-green-dark);">
  <div class="flex flex-col">
    ...
    <div class="flex items-center space-x-3 mt-1">
      ...
      <!-- View As chip for super admin impersonation -->
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // From analytics_scopes (per-election + abstain)
  ...
  function updateCollegeSelectorOptions(breakdownType) {
      ...
      // itabi muna kung ano ang huling napili (kung meron)
      const lastSelected = turnoutCollegeSelect.dataset.selected || turnoutCollegeSelect.value || 'all';
      ...
      if (breakdownType === 'college') {
          ...
          turnoutCollegeSelect.appendChild(opt);
          <?php endif; ?>
      } else if (breakdownType === 'department' || breakdownType === 'course') {
          <?php if ($isCSGAdmin): ?>
          const colleges = <?= json_encode($allColleges) ?>;
          colleges.forEach(college => {
              const opt = document.createElement('option');
      ...
      // ibalik ang dating napili kung existing pa sa options
      const hasOption = Array.from(turnoutCollegeSelect.options)
          .some(o => o.value === lastSelected);

      if (hasOption) {
          turnoutCollegeSelect.value = lastSelected;
      }

      // i-store sa dataset ang kasalukuyang value
      turnoutCollegeSelect.dataset.selected = turnoutCollegeSelect.value;
  }

  function updateCollegeSelectorVisibility() {
    if (!turnoutCollegeSelector || !breakdownSelect) return;
    const b = breakdownSelect.value;
    if (b === 'college' || b === 'department' || b === 'course') {
    ...
    if (currentDataSeries === 'elections') {
      ...
      <?php if ($isCSGAdmin): ?>
        ...
        if (selectedCollege === 'all') {
          ...
          labels   = colleges.map(r => getDepartmentAbbrevJS(collegeFullNameMap[r.college] || r.college));
          ...
          labels   = filtered.map(r => getDepartmentAbbrevJS(collegeFullNameMap[r.college] || r.college));
        ...
        leftLabel = 'Number of Students';
        titleText = `Eligible Students vs Turnout Rate (Departments of ${selectedCollege})`;
      }
      <?php endif; ?>
  ...
  if (turnoutCollegeSelect) {
      turnoutCollegeSelect.addEventListener('change', function () {
          // i-save kung anong college ang pinili
          turnoutCollegeSelect.dataset.selected = this.value;
          // tapos i-re-render ang chart/table
          renderElectionsVsTurnout();
      });
  }
  ...
  if (barCtx) {
    ...
    function getBarChartDataDetailed(breakdownType, college) {
      ...
      if (breakdownType === 'all') {
        ...
        data = barDataDetailed[breakdownType].filter(item => item.college_name === college);
    ...
    function updateDetailedTable(breakdownType, college) {
      ...
      if (breakdownType === 'all') {
        ...
        tableHeader.innerHTML = `
          ...
          <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Voters</th>
        `;
        const departmentData = barDataDetailed.department.filter(item => item.college_name === college);
        ...
        tableHeader.innerHTML = `
          ...
          <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Voters</th>
        `;
        const courseData = barDataDetailed.course.filter(item => item.college_name === college);
...
// global, used by onclick in HTML
