...
// Fetch all votes for this voter in this election,
// including abstain rows (is_abstain = 1) and resolved position names.
$stmt = $pdo->prepare("
    SELECT
        v.position_id,
        v.position_name,
        ...
        v.candidate_id,
        v.vote_datetime,
        COALESCE(p.position_name, v.position_name) AS full_position_name,
        c.first_name,
        c.middle_name,
        c.last_name
    FROM votes v
    ...
    ORDER BY full_position_name, v.vote_datetime
");
$stmt->execute([$election_id, $_SESSION['user_id']]);
$voteRows = $stmt->fetchAll();

// If for some reason there are no votes recorded, just redirect
if (!$voteRows) {
    header("Location: voters_dashboard.php?error=no_votes_found");
    exit();
}

// Build $votesByPosition = [ 'President' => ['Candidate A', 'Candidate B'], 'VP' => ['Abstain'], ... ]
$votesByPosition = [];

foreach ($voteRows as $row) {
    $positionName = $row['full_position_name'] ?: 'Unknown Position';

    if (!isset($votesByPosition[$positionName])) {
        $votesByPosition[$positionName] = [];
    }

    if ((int)$row['is_abstain'] === 1) {
        // For abstain rows, just record 'Abstain' (and override any previous entry if somehow present).
        if (empty($votesByPosition[$positionName])) {
            $votesByPosition[$positionName][] = 'Abstain';
        }
    } else {
        // Normal candidate vote
        if (!empty($row['candidate_id'])) {
            $candidateName = $row['first_name'];
            if (!empty($row['middle_name'])) {
                $candidateName .= ' ' . strtoupper(substr(trim($row['middle_name']), 0, 1)) . '.';
            }
            $candidateName .= ' ' . $row['last_name'];
...
function sendVoteReceiptEmail($voter, $election, $votesByPosition) {
    ...
    try {
        ...
        $mail->Password   = 'ghdumnwrjbphujbs'; // make sure this is your app password, not real account password
...
<head>
  ...
  <style>
    ...
    .notification-text { font-size: 16px; line-height: 1.4; }
    .btn-container {
      display: flex;
    ...
    .btn-email:hover { background-color: #f3f4f6; }
    .btn-dashboard {
      background-color: var(--cvsu-green-light);
      color: white;
    }
    .btn-dashboard:hover { background-color: var(--cvsu-green-dark); }
          ...
          <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <?php foreach ($votesByPosition as $position => $candidates): ?>
                <?php if (in_array('Abstain', $candidates, true)): ?>
                  <div class="vote-item no-vote-item rounded-xl p-6">
                    <h4 class="text-lg font-semibold mb-3"><?= htmlspecialchars($position) ?></h4>
                    <div class="flex items-center mb-2">
                      <i class="fas fa-minus-circle text-red-300 mr-2"></i>
                      <span>Abstain (no candidate selected)</span>
                    </div>
                  </div>
                <?php else: ?>
