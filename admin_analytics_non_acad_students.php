  ...
  turnoutChartInstance = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: `Voter Turnout by ${currentState.breakdownType === 'college' ? 'College' : 'Department'}`,
      ...
      scales: {
        ...
        x: {
          ...
          title: {
            display: true,
            text: currentState.breakdownType === 'college' ? 'College' : 'Department',
            font: { size: 14, weight: 'bold' }
          }
        }
      },
...
function renderCtxYearTable() {
  ...
  if (!labels || labels.length === 0) {
    container.innerHTML = `
      <div class="table-no-data">
        <i class="fas fa-table text-gray-400 text-4xl mb-3"></i>
        <p class="text-gray-600 text-lg">No Non-Academic Student turnout data available.</p>
      </div>`;
    return;
  }
