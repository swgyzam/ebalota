...
// Verify this is a Others-Default Admin
...
if (!$electionId) {
    header('Location: admin_analytics_default.php');
    exit();
}

// Fetch election details (only Others-Default elections)
 $stmt = $pdo->prepare("SELECT * FROM elections WHERE election_id = ? AND target_position = 'others-default'");
...
// For Others-Default elections, we only count voters with is_default_member=1 AND migs_status=1
 $conditions = ["role = 'voter'", "is_default_member = 1", "migs_status = 1"];
 $params = [];
...
// Breakdown by Position (Academic and Non-Academic)
 $sql_position = "
   ...
   WHERE u.role = 'voter' AND u.is_default_member = 1 AND u.migs_status = 1
   GROUP BY u.position
   ORDER BY u.position
";
    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="bg-white rounded-xl shadow-md overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
          ...
          <p class="text-sm text-gray-500 mt-1">Others-Default Election (MIGS members)</p>
        </div>
      ...
      <!-- Back Button -->
      <div class="mt-6">
        <a href="admin_analytics_default.php" 
