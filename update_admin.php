...
// Get current scope details (if any)
$stmt = $pdo->prepare("SELECT * FROM admin_scopes WHERE user_id = ?");
$stmt->execute([$user_id]);
$scopeData = $stmt->fetch();

// We'll recompute scope_details from POST for Academic/Non-Ac Employee categories
$scope_details    = [];
$assigned_scope   = '';
$assigned_scope_1 = '';
...
switch ($scope_category) {
    ...
    case 'Non-Academic-Student': {
        // Scope is static for now: all non-academic student orgs
...
try {
    ...
    // Update or insert admin_scopes (now including scope_value)
    if ($scopeData) {
    ...
    if ($email_changed) {
        try {
                ...
                <body>
                    <div class='container'>
                        ...
                        <div class='content'>
                            ...
                            <div class='reminder'>
                                <strong>Important:</strong> Your email address has been changed from <strong>$old_email</strong> to <strong>$email</strong>.
                            </div>
                        </div>
            ...
            $mail->AltBody = "Your admin account has been updated. Title: $admin_title, Name: $first_name $last_name, Email: $email, Scope: $scope_description. Old email: $old_email.";

            $mail->send();
        } catch (Exception $e) {
            error_log("Email notification failed (update_admin): " . $mail->ErrorInfo);
        }
    }
