...
for ($i = 0; $i < 12; $i++) {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) as count 
                           ...
                           WHERE role = 'voter' AND position = 'student' AND UPPER(TRIM(department)) = ?
                             AND created_at BETWEEN ? AND ?");
    $stmt->execute([$scope, $monthStart, $monthEnd]);
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) as count 
                           ...
                           WHERE role = 'voter' AND position = 'student' AND UPPER(TRIM(department)) = ?
                             AND created_at BETWEEN ? AND ?");
    $stmt->execute([$scope, $prevMonthStart, $prevMonthEnd]);
...
// --- Fetch Year-over-Year Cumulative Comparison Data - unchanged ---
 $yearlyData = [];
 $cumulativeSelected = 0;
 $cumulativePrevious = 0;

// Check if previous year data exists in the database
 $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM users WHERE role = 'voter' AND position = 'student' AND UPPER(TRIM(department)) = ? AND YEAR(created_at) = ?");
 $stmt->execute([$scope, $previousYear]);
...
<main class="flex-1 pt-20 px-8 ml-64">
  ...
  <div class="analytics-section mb-8 bg-white rounded-xl shadow-lg overflow-hidden">
    ...
    <div class="border-t p-6">
      <h3 class="text-xl font-semibold text-gray-800 mb-6">Student Growth Analytics</h3>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div class="p-4 rounded-lg" style="background-color: rgba(59, 130, 246, 0.05);">
          <h3 class="text-lg font-semibold text-gray-800 mb-4">Student Registration Trend</h3>
          <div class="chart-container">
      ...
      <div class="mt-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div class="p-4 rounded-lg border" style="background-color: rgba(59,130,246,0.05); border-color:#3B82F6;">
            <div class="flex items-center">
              <div class="p-3 rounded-lg mr-4 bg-blue-500"><i class="fas fa-chart-line text-white text-xl"></i></div>
              <div>
                <p class="text-sm text-blue-600">Average Growth</p>
                <p class="text-2xl font-bold text-blue-800"><?= round(array_sum($historicalGrowth) / (count($historicalGrowth) ?: 1), 1) ?>%</p>
              </div>
            </div>
          </div>
          <div class="p-4 rounded-lg border" style="background-color: rgba(16,185,129,0.05); border-color:#10B981;">
            <div class="flex items-center">
              <div class="p-3 rounded-lg mr-4 bg-green-500"><i class="fas fa-arrow-up text-white text-xl"></i></div>
              <div>
                <p class="text-sm text-green-600">Highest Growth</p>
                <p class="text-2xl font-bold text-green-800"><?= max($historicalGrowth) ?>%</p>
              </div>
            </div>
          </div>
          <div class="p-4 rounded-lg border" style="background-color: rgba(239,68,68,0.05); border-color:#EF4444;">
            <div class="flex items-center">
              <div class="p-3 rounded-lg mr-4 bg-red-500"><i class="fas fa-arrow-down text-white text-xl"></i></div>
              <div>
                <p class="text-sm text-red-600">Lowest Growth</p>
                <p class="text-2xl font-bold text-red-800"><?= min($historicalGrowth) ?>%</p>
              </div>
            </div>
          </div>
          <div class="p-4 rounded-lg border" style="background-color: rgba(139,92,246,0.05); border-color:#8B5CF6;">
            <div class="flex items-center">
              <div class="p-3 rounded-lg mr-4 bg-purple-500"><i class="fas fa-calendar-check text-white text-xl"></i></div>
              <div>
                <p class="text-sm text-purple-600">Positive Months</p>
                <p class="text-2xl font-bold text-purple-800">
                  <?= count(array_filter($historicalGrowth, fn($r) => $r > 0)) ?>
                </p>
              </div>
            </div>
          </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">New Students</th>
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <?php foreach ($tableData as $data): ?>
                  <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      <?= number_format($data['previous']) ?>
                      <span class="text-xs text-gray-500">(<?= $data['previous_month_label'] ?>)</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <span class="<?= $data['growth'] > 0 ? 'text-green-600' : ($data['growth'] < 0 ? 'text-red-600' : 'text-gray-600') ?>">
                        <?= ($data['growth'] > 0 ? '+' : '') . $data['growth'] ?>%
                      </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <i class="fas <?= $data['growth'] > 0 ? 'fa-arrow-up text-green-600' : ($data['growth'] < 0 ? 'fa-arrow-down text-red-600' : 'fa-minus text-gray-600') ?>"></i>
                    </td>
                  </tr>
                <?php endforeach; ?>
              </tbody>
        ...
        <div class="mt-6 p-4 rounded-lg" style="background-color: rgba(30, 111, 70, 0.03);">
          ...
          <p class="text-gray-600">
            <?php
            $positiveCount = count(array_filter($historicalGrowth, fn($rate) => $rate > 0));
            $negativeCount = count(array_filter($historicalGrowth, fn($rate) => $rate < 0));
            $avgGrowth = round(array_sum($historicalGrowth) / (count($historicalGrowth) ?: 1), 1);

            if ($avgGrowth > 5) {
                echo "📈 Strong positive growth trend with an average of {$avgGrowth}%. ";
            } elseif ($avgGrowth > 0) {
                echo "📊 Moderate positive growth at {$avgGrowth}%. ";
            } elseif ($avgGrowth < -5) {
                echo "📉 Significant decline averaging {$avgGrowth}%. ";
            } else {
                echo "➡️ Stable growth pattern with minimal fluctuations. ";
            }
            echo "Out of the last 12 months, {$positiveCount} were positive and {$negativeCount} declined.";
            ?>
          </p>
        </div>
      </div>
    ...
    <div class="border-t p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="p-4 rounded-lg border" style="background-color: rgba(99,102,241,0.05); border-color:#6366F1;">
          <div class="flex items-center">
            <div class="p-3 rounded-lg mr-4 bg-indigo-500"><i class="fas fa-calendar-alt text-white text-xl"></i></div>
            <div>
        ...
        <div class="p-4 rounded-lg border" style="background-color: rgba(139,92,246,0.05); border-color:#8B5CF6;">
          <div class="flex items-center">
            <div class="p-3 rounded-lg mr-4 bg-purple-500"><i class="fas fa-calendar text-white text-xl"></i></div>
            <div>
        ...
        <div class="p-4 rounded-lg border" style="background-color: rgba(16,185,129,0.05); border-color:#10B981;">
          <div class="flex items-center">
            <div class="p-3 rounded-lg mr-4 bg-green-500"><i class="fas fa-percentage text-white text-xl"></i></div>
            <div>
              <p class="text-sm text-green-600">Yearly Growth</p>
              <p class="text-2xl font-bold text-green-800">
                <?= $previousYearTotal > 0 ? (($yearlyGrowthRate>0?'+':'').$yearlyGrowthRate.'%') : '0%' ?>
              </p>
            </div>
          </div>
        </div>
        <div class="p-4 rounded-lg border" style="background-color: rgba(59,130,246,0.05); border-color:#3B82F6;">
          <div class="flex items-center">
            <div class="p-3 rounded-lg mr-4 bg-blue-500"><i class="fas fa-chart-bar text-white text-xl"></i></div>
            <div>
              <p class="text-sm text-blue-600">Avg Monthly</p>
              <p class="text-2xl font-bold text-blue-800"><?= round($selectedYearTotal/12,0) ?></p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <div class="p-4 rounded-lg" style="background-color: rgba(99,102,241,0.05);">
          <h3 class="text-lg font-semibold text-gray-800 mb-4">Cumulative Student Growth: <?= $selectedYear ?> vs <?= $selectedYear - 1 ?></h3>
          <div class="chart-container">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="<?= $trendColor ?>"><?= ($data['growth']>0?'+':'').$data['growth'] ?>%</span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap"><i class="fas <?= $trendIcon ?> <?= $trendColor ?>"></i></td>
              </tr>
              <?php endforeach; ?>
            </tbody>
          </table>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const yearlySelected = <?= json_encode(array_column($yearlyData, 'selected_year')) ?>;
  const yearlyPrev = <?= json_encode(array_column($yearlyData, 'previous_year')) ?>;
  const yearlyGrowth = <?= json_encode(array_column($yearlyData, 'growth')) ?>;
  ...
  if (growthRateCtx && !growthRateChartInstance) {
    growthRateChartInstance = new Chart(growthRateCtx, {
      type:'line',
      data:{ labels: monthLabels, datasets:[{ label:'Growth Rate (%)', data:growthRates, borderColor:'#FFD166', backgroundColor:'rgba(255,209,102,0.1)', borderWidth:3, pointBackgroundColor:'#FFD166', pointBorderColor:'#fff', pointBorderWidth:2, pointRadius:5, pointHoverRadius:7, fill:true, tension:0.4 }]},
      options:{ responsive:true, maintainAspectRatio:false, plugins:{ legend:{display:false}, tooltip:{ backgroundColor:'rgba(0,0,0,0.8)', titleFont:{size:14}, bodyFont:{size:13}, padding:12, callbacks:{ label:(c)=>`Growth: ${c.raw}%` }}},
        scales:{ y:{ beginAtZero:true, ticks:{ callback:(v)=> v+'%' }, grid:{ color:'rgba(0,0,0,0.05)' }}, x:{ grid:{display:false}}}
      }
    });
  }
  ...
  if (registrationTrendCtx && !registrationTrendChartInstance) {
    registrationTrendChartInstance = new Chart(registrationTrendCtx, {
      type:'bar',
      data:{ labels: monthLabels, datasets:[{ label:'New Students', data:voterCounts, backgroundColor:'#3B82F6', borderColor:'#1D4ED8', borderWidth:1, borderRadius:4, barThickness:'flex', maxBarThickness:40, hoverBackgroundColor:'#2563EB', hoverBorderColor:'#1D4ED8' }]},
      options:{ responsive:true, maintainAspectRatio:false, plugins:{ legend:{display:false}, tooltip:{ backgroundColor:'rgba(0,0,0,0.8)', titleFont:{size:14}, bodyFont:{size:13}, padding:12, callbacks:{ label:(c)=>`New Students: ${c.raw}` }}},
        scales:{ y:{ beginAtZero:true, ticks:{precision:0, font:{size:12}}, grid:{color:'rgba(0,0,0,0.1)'}, title:{display:true, text:'Number of New Students', font:{size:14, weight:'bold'}}}, x:{ ticks:{font:{size:12}, maxRotation:45, minRotation:45}, grid:{display:false}, title:{display:true, text:'Month', font:{size:14, weight:'bold'}}}},
        animation:{ duration:1000, easing:'easeOutQuart' }
      }
    });
  }
  ...
  if (yearComparisonCtx && !yearComparisonChartInstance) {
    yearComparisonChartInstance = new Chart(yearComparisonCtx, {
      ...
      data:{ labels:['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'],
        datasets:[
          { label:'<?= $selectedYear ?>', data:yearlySelected, borderColor:'#1E6F46', backgroundColor:'rgba(30,111,70,0.1)', borderWidth:3, pointBackgroundColor:'#1E6F46', pointBorderColor:'#fff', pointBorderWidth:2, pointRadius:5, pointHoverRadius:7, fill:false, tension:0.4 },
          { label:'<?= $selectedYear - 1 ?>', data:yearlyPrev, borderColor:'#37A66B', backgroundColor:'rgba(55,166,107,0.1)', borderWidth:3, pointBackgroundColor:'#37A66B', pointBorderColor:'#fff', pointBorderWidth:2, pointRadius:5, pointHoverRadius:7, fill:false, tension:0.4 }
        ]},
      options:{ responsive:true, maintainAspectRatio:false, plugins:{ legend:{ position:'top', labels:{ font:{size:12}, padding:15, usePointStyle:true, pointStyle:'circle'}}, tooltip:{ backgroundColor:'rgba(0,0,0,0.8)', titleFont:{size:14}, bodyFont:{size:13}, padding:12, callbacks:{ label:(c)=> `${c.dataset.label}: ${c.raw} cumulative students` }}},
        scales:{ y:{ beginAtZero:true, ticks:{precision:0, font:{size:12}}, grid:{color:'rgba(0,0,0,0.05)'}, title:{display:true, text:'Cumulative Number of Students', font:{size:14, weight:'bold'}}}, x:{ grid:{display:false}, title:{display:true, text:'Month', font:{size:14, weight:'bold'}}}}
      }
    });
  }
  ...
  if (monthlyGrowthCtx && !monthlyGrowthChartInstance) {
    monthlyGrowthChartInstance = new Chart(monthlyGrowthCtx, {
      type:'bar',
      data:{ labels:['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'],
        datasets:[{ label:'Growth Rate (%)', data:yearlyGrowth, backgroundColor: yearlyGrowth.map(r => r>0?'#1E6F46':(r<0?'#EF4444':'#6B7280')), borderColor: yearlyGrowth.map(r => r>0?'#154734':(r<0?'#DC2626':'#4B5563')), borderWidth:1, borderRadius:4, barThickness:'flex', maxBarThickness:40, hoverBackgroundColor: yearlyGrowth.map(r => r>0?'#37A66B':(r<0?'#DC2626':'#4B5563')) } ]},
      options:{ responsive:true, maintainAspectRatio:false, plugins:{ legend:{display:false}, tooltip:{ backgroundColor:'rgba(0,0,0,0.8)', titleFont:{size:14}, bodyFont:{size:13}, padding:12, callbacks:{ label:(c)=> `Cumulative Growth: ${c.raw}%` }}},
        scales:{ y:{ beginAtZero:true, ticks:{ callback:(v)=> v+'%' , font:{size:12}}, grid:{color:'rgba(0,0,0,0.1)'}, title:{display:true, text:'Cumulative Growth Rate (%)', font:{size:14, weight:'bold'}}}, x:{ ticks:{font:{size:12}, maxRotation:45, minRotation:45}, grid:{display:false}, title:{display:true, text:'Month', font:{size:14, weight:'bold'}}}},
        animation:{ duration:1000, easing:'easeOutQuart' }
      }
    });
  }
